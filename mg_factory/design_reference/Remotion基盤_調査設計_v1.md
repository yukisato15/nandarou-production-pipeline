...
> **続編あり(2026-07-14)**: 本書はRemotionの**第一の役割=情報カットの自動生成**を扱う。
> 検証の結果、Remotionは**第二の役割=資料モンタージュ編集台**(実写+静止画+紙+数字+注釈の
> 合成)としても使えることが分かった。そちらは `資料モンタージュ設計_v1.md` に分離した。
> 両者は同じ基盤(theme/stages/registry/zod/カット単位書き出し)を共有する。
...
## 6. 使い分けの最終整理(2026-07-14更新)

| カット種別 | 作り方 |
|---|---|
| グラフ・カウンター・年表・引用・★宣言・タイトル | **Remotion 情報カット**(JsonScenes+レジストリ) |
| 資料モンタージュ(実写+静止画+紙+数字+注釈の合成・VOX的) | **Remotion モンタージュ**(MontageScene。`資料モンタージュ設計_v1.md`) |
| 分解図ボード(質感重視: C13型) | AEマスター複製 or Remotionモンタージュ(質感要求が高ければAE) |
| 偽サムネ・偽UI静止画 | Canva/Photoshop+AI画像(既存フロー) |
| 「その場の空気」実写 | Envato / 自分で撮影 |
| 歴史・抽象・再現不能な情景 | 生成AI(Runway / Gemini) |
| 最終合成・音・呼吸 | Premiere(+必要カットのみAE) |

補足: Envato等の既存素材や半完成の実写は、生で貼るだけでなく**Remotionモンタージュ上で
再構成**してよい(クロップ・グレード・切り抜き・合成)。生成AIが高コスト/不安定になる
「正確な資料の図示」はモンタージュへ寄せ、生成AIは主力のまま歴史・抽象に集中させる。
