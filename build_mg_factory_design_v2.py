#!/usr/bin/env python3
"""番組用デザインシステム v2 のAE JSXを作る。

v0.2は「コンテからAEへ確実にレイヤーを置く」ことを検証するための
機能試作だった。このビルダーはその配線を流用しつつ、見た目を先に
確定するための小さなデザイン試作である。

対象はC01 / C13 / C25のみ。3つが番組の画として成立してから、他の
テンプレートへ横展開する。既存のgenerate_mg_factory.jsxは変更しない。
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="MG Factory Design v2 JSXを生成")
    parser.add_argument("input", type=Path, help="ep01_mg.json")
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("mg_factory/generated/generate_mg_factory_design_v2.jsx"),
    )
    return parser.parse_args()


def build_payload(source: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(source.get("project"), dict) or not isinstance(source.get("cuts"), list):
        raise ValueError("MG JSONのproject/cutsがありません")
    if (source["project"].get("width"), source["project"].get("height")) != (1920, 1080):
        raise ValueError("Design v2は1920×1080専用です")
    by_cut = {cut.get("cut"): cut for cut in source["cuts"]}
    missing = [cut for cut in DESIGN_CUTS if cut not in by_cut]
    if missing:
        raise ValueError("Design v2対象カットがありません: " + ", ".join(missing))
    selected = [by_cut[cut] for cut in DESIGN_CUTS]
    for cut in selected:
        if not isinstance(cut.get("params"), dict) or not cut["params"]:
            raise ValueError(f"{cut['cut']}: paramsがありません")
    return {"project": source["project"], "cuts": selected}
...
def main() -> int:
    ...
    try:
        ...
        return 1
    jsx = JSX_TEMPLATE.replace("__PAYLOAD__", json.dumps(payload, ensure_ascii=False, separators=(",", ":")), 1)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(jsx, encoding="utf-8-sig")
    print("=== MG Factory Design v2 ===")
    print("対象: " + ", ".join(DESIGN_CUTS))
